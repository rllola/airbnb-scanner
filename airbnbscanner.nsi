...
!define DESCRIPTION "An IP camera scanner"
...
section "install"
  ...
  FileOpen $0 "$APPDATA\${APPNAME}\config.ini" w
  FileWrite $0 "[theme]$\n"
  FileWrite $0 "DarkMode = false$\n"
  FileClose $0
  ...
  createShortCut "$SMPROGRAMS\${APPNAME}\${APPNAME}.lnk" "$INSTDIR\AirbnbScanner.exe" "" "$INSTDIR\${MUI_ICON}"
